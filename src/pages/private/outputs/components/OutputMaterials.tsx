...
interface OutputMaterialsProps {
    updateOutputs: () => void;
    resetSelectedOption: () => void;
    handleAlert: (severity: "success" | "info" | "warning" | "error", text: string) => void;
}

export const OutputMaterials = ({ updateOutputs, resetSelectedOption, handleAlert }: OutputMaterialsProps) => {
    ...
    const handleSubmit = (e: React.ChangeEvent<HTMLInputElement>) => {
        e.preventDefault();
        if (selectedMaterials.some((material, index) => quantity[index] > material.stock)) {
            handleAlert('error', 'La cantidad no puede superar al stock actual');
            return;
        }
        setOpenDialog(true);
    }
    ...
    const newOutput = async () => {
        try {
            ...
            console.log(data)
            handleAlert('success', 'Egreso registrado con éxito')
        } catch (error) {
            console.error(error)
            handleAlert('error', 'Error al registrar egreso: ' + (error as Error).message)
        } finally {
            setOpenBack(false);
            updateOutputs()
            resetSelectedOption()
        ...
        <Box height='70vh' id='step-2' component='form' onSubmit={handleSubmit}>
                ...
                <Box bgcolor='background.default' borderRadius={2.5}>
                    {selectedMaterials.map((material, index) => (
                        <Box key={index} display='flex' flexDirection='column' justifyContent='center' gap={1} border={1} borderColor='background.paper' borderRadius={2.5} padding={1} mb={2}>
                            <Box display='flex' justifyContent='space-around' flexDirection={{ xs: 'column', sm: 'row' }}  gap={1}>
                                <Autocomplete
                                    options={materials}
                                ...
                                <TextField
                                    ...
                                    fullWidth
                                    error={quantity[index] > material?.stock}
                                    helperText={material.name && quantity[index] > material?.stock ? 'No puede superar al stock actual' : ''}
                                    inputProps={{
                                        maxLength: 8
                                    }}
                                />
            ...
            >
                <DialogTitle id="alert-dialog-title">
                    {'¿Desea confirmar el siguiente egreso?'}
                </DialogTitle>
